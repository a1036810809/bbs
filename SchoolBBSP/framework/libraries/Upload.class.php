...
class Upload{
	...
	public function baseToImage($data){
		$image = explode(',',$data->base64);
		$image = $image[1];
		...
		//文件重命名,由当前日期 + 随机数 + 后缀名
		$file_name = date('YmdHis').uniqid().strrchr($data->name, '.');

		$r = file_put_contents($this->path . $sub_path . $file_name, base64_decode($image));
		if (!$r) {
